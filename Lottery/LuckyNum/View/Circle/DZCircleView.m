...
#import "DZCircleBgView.h"
#import "DZCircleItemView.h"

// 1秒种转多少度
#define DZCircleRotateSpeedPerSecond (M_PI_4/2)
#define DZTransform2Angle(transform) atan2(transform.b, transform.a)

@interface DZCircleView()
{
    DZCircleBgView *_bgView;
...
- (void)setCircleType:(DZCircleViewType)circleType
{
    _circleType = circleType;
    
    // 更换背景view内部使用的图片
    if (circleType == DZCircleViewTypeAstrology) {//星座
...
{
    frame.size = CGSizeMake(DZCircleWH, DZCircleWH);
    [super setFrame:frame];
}

- (void)setBounds:(CGRect)bounds
{
    bounds.size = CGSizeMake(DZCircleWH, DZCircleWH);
...
- (void)setupBg
{
    DZCircleBgView *bgView = [DZCircleBgView circleBgView];
...
- (void)start
{
    ...
    CGFloat angle = DZTransform2Angle(_bgView.selectedBtn.transform);
...
//    CGAffineTransformMakeRotation(CGFloat angle)
    ...
    // 时间 * 速度 == 角度
    CGFloat angle = timer.duration * DZCircleRotateSpeedPerSecond;
    // 旋转一定的角度
