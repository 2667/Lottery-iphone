...
const int DZCircleWH = 286;

@interface DZCircleBgView()

@end

@implementation DZCircleBgView
#pragma mark - 生命周期方法
...
{
    for (int i = 0; i < 12; i++) {
        DZCircleItemView *btn = [DZCircleItemView buttonWithType:UIButtonTypeCustom];
        btn.tag = i;
        
        // 设置位置
        CGFloat posX = DZCircleWH * 0.5;
        CGFloat posY = posX;
...
{
    for (int i = 0; i<12; i++) {
        DZCircleItemView *btn = self.subviews[i];
...
- (void)btnClick:(DZCircleItemView *)btn
{
...
- (void)drawRect:(CGRect)rect
{
    ...
    CGFloat innerCircleWH = DZCircleWH - innerCircleXY * 2;
